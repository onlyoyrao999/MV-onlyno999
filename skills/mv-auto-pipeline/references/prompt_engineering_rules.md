...
### 对应 Negative Prompt:
```
singing, mouth open, lip-sync, talking, speaking, vocalizing, open lips, moving mouth, cartoon, 3d render, distorted anatomy, jitter, flicker
```
...
| **04** | `CHECK_NO_TALK_VERB`| **CRITICAL** | 严禁出现 `saying`, `talking`, `speaking`, `chatting` 等对白词汇 |
...
| **07** | `CHECK_NEG_LIP`     | **CRITICAL** | 非口型段 Negative 必须压制 `singing, mouth open, lip-sync` |
